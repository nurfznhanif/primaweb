...
<div class="col-lg-8">
    <form method="POST" action="/dashboard/dokter-jadwal">
        @csrf
        <h4>{{$member->nama}}</h4> <span class="text-muted">{{$member->poliklinik->poliklinik}}</span>

        <div class="mb-3 mt-3">
            <input type="hidden" class="form-control" name="dokter_id" id="dokter_id" value="{{ $member->id}}" autofocus>
        </div>
        <div class="mb-3">
            <input type="hidden" class="form-control" name="poliklinik_id" id="poliklinik_id" value="{{ $member->poliklinik_id}}" autofocus>
        </div>
