                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr class="align-middle">
                                <td><img src="{{ asset('images/dokter-image/'.$member->image) }}" alt="" width="100"></td>
                                <td>{{$member->nama}}</td>
                                <td>{{$member->jenis_kelamin}}</td>
                                <td>{{$member->no_hp}}</td>
                                <td>{{$member->email}}</td>
                                <td>
                                    <a href="/dashboard/dokter-jadwal/{{$member->slug}}" class="btn btn-dark" title="Tambah Jadwal"><i class="bi bi-calendar-plus"></i></a>
                                    <a href="/dashboard/dokter/{{$member->slug}}" class="btn btn-primary" title="Detail"><i class="bi bi-eye"></i></a>
                                    <a href="/dashboard/dokter/{{$member->slug}}/edit" class="btn btn-warning" title="Edit"><i class="bi bi-pencil-square"></i></a>

                                    <form action="/dashboard/dokter/{{$member->slug}}" method="POST" class="d-inline">
                                        @method('delete')
                                        @csrf
